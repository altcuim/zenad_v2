...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ZNDMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ZNDMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ZNDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zenad-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ZNDMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ZNDMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadZNDMiner() exception");
    } catch (...) {
        LogPrintf("ThreadZNDMiner() exception");
    }

    LogPrintf("ThreadZNDMiner exiting\n");
}
