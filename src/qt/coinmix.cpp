...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::ZND)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeZNDAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeZNDAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeZNDAmount = strAnonymizeZNDAmount.remove(strAnonymizeZNDAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZNDAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeZNDAmount * COIN) nMaxToAnonymize = nAnonymizeZNDAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeZNDAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeZNDAmount));
        strAnonymizeZNDAmount = strAnonymizeZNDAmount.remove(strAnonymizeZNDAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZNDAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
