...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ZND address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no ZND: URI
...
{
    // Convert zenad:// to zenad:
    //
    //    Cannot handle this later, because zenad:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open zenad.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ZND.lnk";
}
...
{
    return GetAutostartDir() / "zenad.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a zenad.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the zenad app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add zenad app to startup item list
